import { ModelOption, ExpertResult, MessageAttachment } from '../../types.js';
import { getExpertSystemInstruction } from './prompts.js';
import { withRetry } from '../utils/retry.js';
import { generateContentStream as generateOpenAIStream } from './openaiClient.js';
...
export const streamExpertResponse = async (
  ...
  if (isGoogle) {
    ...
    logger.info("Expert", `Starting OpenAI stream for ${expert.role}`, { model, temperature: expert.temperature });
    ...
    try {
      ...
      logger.error("Expert", `Stream error for expert ${expert.role} (OpenAI)`, {

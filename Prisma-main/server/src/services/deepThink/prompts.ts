import { ExpertResult } from '../../types.js';
...
export const getExpertSystemInstruction = (role: string, description: string, context: string): string => {
  return `You are a ${role}. ${description}. Context: ${context}`;
};

export const getSynthesisPrompt = (recentHistory: string, query: string, expertResults: ExpertResult[]): string => {
  return `
